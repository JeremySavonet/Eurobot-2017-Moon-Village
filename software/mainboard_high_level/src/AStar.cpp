...
void AStar::calculateCost( QPair< uint, uint > node )
{
    ...
    else if( _heuristic == diagonal )
    {
        h = std::max(abs(x - destX), abs(y - destY));
    }
    else if( _heuristic == newH )
    {
        h = std::ceil(euc) - euc;
    }
    ...
    QList< NodeState >* changes )
{
    ...
    if( _matrix[ x ][ y ].type == WALLNODE ||
        _matrix[ x ][ y ].type == VISITEDNODE )
    {
        return;

    }
    else if( _matrix[ x ][ y ].type == WAYNODE )
    {
        _matrix[ x ][ y ].type = OPENEDNODE;
    ...
    if( changes != NULL )
    {
        ...
        if( originX == -1 )
        {
            if( originY == -1 )
            {
                nodeState.ancestor = UPLEFT;
            }
            else if( originY == 0 )
            {
                nodeState.ancestor = LEFT;
            }
            else
            {
                nodeState.ancestor = DOWNLEFT;
            }
        }
        else if( originX == 1 )
        {
            if( originY == -1 )
            {
                nodeState.ancestor = UPRIGHT;
            }
            else if( originY == 0 )
            {
                nodeState.ancestor = RIGHT;
            }
            else
            {
                nodeState.ancestor = DOWNRIGHT;
        ...
        else
        {
            if( originY == -1 )
            {
                nodeState.ancestor = UP;
            }
            else
            {
                nodeState.ancestor = DOWN;
            }
        }
...
void AStar::freeNodes()
{
    ...
    for( uint i = 0; i < _width;  ++i )
    {
        ...
        {
            if( _matrix[ i ][ j ].type != WALLNODE )
            {
                _matrix[ i ][ j ].type = WAYNODE;
            }
        }
    }
...
void AStar::search( bool saveChanges )
{
    ...
    while( _matrix[ _end.first ][ _end.second ].type != OPENEDNODE )
    {
