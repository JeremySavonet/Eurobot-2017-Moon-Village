...
static ssize_t leds_read( struct file* file,
                          char* buffer,
                          size_t len,
                          loff_t* offset );

static ssize_t leds_write( struct file* file,
                           const char* buffer,
                           size_t len,
                           loff_t* offset );

// An instance of this structure will be created
// for every custom_led IP in the system
...
// (In our case, this should only get called for the one instantiation of
// the Custom LEDs module)
static int leds_probe( struct platform_device* pdev )
{
    int ret_val = -EBUSY;
    ...
    // (this is used to create a character file in userspace)
    dev->miscdev.minor = MISC_DYNAMIC_MINOR; // Dynamic choose a minor number
    ...
    // Give a pointer to the instance-specific data to the
    // generic platform_device structure so we can access this data later on
    // (for instance, in the read and write functions)
...
// This function gets called whenever a read operation
// occurs on one of the character files
static ssize_t leds_read( struct file* file,
                          char* buffer,
                          size_t len,
                          loff_t* offset )
{
    ...
    // "file"s private data to the appropriate miscdevice structure.
    // We then use the container_of macro to get the structure that miscdevice
    // is stored inside of (which is our custom_leds_dev structure that has
    ...
    struct custom_leds_dev* dev = container_of( file->private_data,
                                                struct custom_leds_dev,
                                                miscdev );

    // Give the user the current led value
    success = copy_to_user( buffer,
                            & dev->leds_value,
                            sizeof( dev->leds_value ) );
    ...
    if( 0 != success )
    {
        ...
        // Bad address error value.
        // It's likely that "buffer" doesn't point to a good address
        return -EFAULT;
    }

    // "0" indicates End of File, aka, it tells the user process to stop reading
    return 0;
}

// This function gets called whenever a write operation occurs
// on one of the character files
static ssize_t leds_write( struct file* file,
                           const char* buffer,
                           size_t len, loff_t* offset )
{
    ...
    success = copy_from_user( & dev->leds_value,
                              buffer,
                              sizeof( dev->leds_value ) );
    ...
    // (even if we only wrote the first value, this will ensure they don't
    // try to re-write their data)
    return len;
}
...
MODULE_LICENSE( "GPL" );
MODULE_AUTHOR( "Jeremy Savonet <jeremy.savonet@gmail.com>" );
